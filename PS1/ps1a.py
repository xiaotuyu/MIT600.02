...
def load_cows(filename):
    ...
    for line in infile:
        ...
        lineSplit[1] = int(lineSplit[1].strip('\n'))
        cdict[lineSplit[0]] = lineSplit[1]
    return cdict
...
def greedy_cow_transport(cows,limit=10):
    ...
    copy = cows.copy()
    while len(copy) > 0:
        values = sorted(copy.values(), reverse = True)
        trip = []
        weight = 0
        for i in values:
            if (weight + i) <= limit:
    ...
    for partition in get_partitions(cows.keys()):
        ...
        for i in range(len(partition)):
            ...
            for j in partition[i]:
                weight = weight + cows[j]
            trip.append(weight)
        if max(trip) <= 10 and len(trip) < length:
            length = len(trip)
            result = partition
    return result
