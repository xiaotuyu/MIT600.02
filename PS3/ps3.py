...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for x in range(width):
            for y in range(height):
                self.tile[(x, y)] = self.dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        if capacity >= self.tile[(math.floor(pos.x), math.floor(pos.y))]:
            self.tile[(math.floor(pos.x), math.floor(pos.y))] = 0
        else:
            self.tile[(math.floor(pos.x), math.floor(pos.y))] -= capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        cleaned_tiles = list(self.tile.values())
        return cleaned_tiles.count(0)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        return (math.floor(pos.x), math.floor(pos.y)) in self.furniture_tiles
    ...
    def get_random_position(self):
        ...
        """
        tileCopy = self.tile.copy()
        for pair in self.furniture_tiles:
            del tileCopy[pair]
        posList = random.choice(list(tileCopy.keys()))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        pos = self.get_robot_position().get_new_position(self.direct, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        for n in range(num_robots):
            robotList.append(robot_type(room, speed, capacity))
        steps = 0
        while float((room.get_num_cleaned_tiles() / room.get_num_tiles())) < min_coverage:
            for j in robotList:
                j.update_position_and_clean()
